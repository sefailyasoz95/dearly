...
const mockAlbums = [
	{
		...
		family_id: "1",
		cover_image: "https://images.unsplash.com/photo-1602002418082-dd67915aeaa5?q=80&w=500",
		is_active: true,
		count: 24,
	},
	{
		...
		parent_album_id: null,
		family_id: "1",
		cover_image: "https://images.unsplash.com/photo-1464349153735-7db50ed83c84?q=80&w=500",
		is_active: true,
		count: 18,
	},
	{
		...
		parent_album_id: null,
		family_id: "1",
		cover_image: "https://images.unsplash.com/photo-1609220136736-443140cffec6?q=80&w=500",
		is_active: true,
		count: 32,
	},
	{
		...
		parent_album_id: null,
		family_id: "1",
		cover_image: "https://images.unsplash.com/photo-1519340241574-2cec6aef0c01?q=80&w=500",
		is_active: true,
		count: 15,
	},
];
...
export default async function DashboardPage() {
	...
	const familyMembers = [
		...
		},
		{
			...
			avatar: "https://images.unsplash.com/photo-1631680900243-3c207cf5a481?q=80&w=200",
			role: "Member",
		},
		{
			name: "David Smith",
	...
	const { data: user, error } = await supabase.from("profiles").select("*").eq("email", session?.user.email).single();
				...
				<div className='relative overflow-hidden'>
					...
					<div className='relative z-20 container mx-auto px-4 py-16 sm:py-24'>
						<div className='max-w-3xl animate-in fade-in slide-in-from-bottom-10 duration-700'>
							<h1 className='text-4xl md:text-5xl font-bold text-white mb-4'>Welcome back, {user?.first_name}!</h1>
							<p className='text-lg text-white/90'>Today is the perfect day to cherish more family moments.</p>
				...
				<div className='container mx-auto px-4 -mt-16 relative z-30'>
						...
						<div className='bg-card hover:bg-card/90 backdrop-blur-sm text-card-foreground rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 hover:translate-y-[-4px] group'>
							<div className='flex items-center justify-between'>
								<div>
									...
									<span className='block text-3xl font-bold mt-1'>{mockAlbums.length}</span>
								</div>
						...
						<div className='bg-card hover:bg-card/90 backdrop-blur-sm text-card-foreground rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 hover:translate-y-[-4px] group'>
							<div className='flex items-center justify-between'>
								<div>
									...
									<span className='block text-3xl font-bold mt-1'>{mockMedia.length}</span>
								</div>
								<div className='w-12 h-12 bg-accent/20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300'>
						...
						<div className='bg-card hover:bg-card/90 backdrop-blur-sm text-card-foreground rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 hover:translate-y-[-4px] group'>
							<div className='flex items-center justify-between'>
								<div>
									...
									<span className='block text-3xl font-bold mt-1'>{familyMembers.length}</span>
								</div>
								<div className='w-12 h-12 bg-accent/20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300'>
						...
						<div className='bg-card hover:bg-card/90 backdrop-blur-sm text-card-foreground rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 hover:translate-y-[-4px] group'>
							<div className='flex items-center justify-between'>
								<div>
									...
									<span className='block text-3xl font-bold mt-1'>2024</span>
								</div>
								<div className='w-12 h-12 bg-accent/20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300'>
					...
					<div className='mb-16'>
						<div className='flex items-center justify-between mb-6'>
							...
							<Link href='/media' className='text-primary hover:underline text-sm'>
								See all →
							</Link>
						</div>
						...
						<div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
							{mockMedia.slice(0, 3).map((media, index) => (
								<div key={media.id} className='group relative rounded-xl overflow-hidden aspect-[4/3] cursor-pointer'>
									<Image
										src={media.url}
									...
									<div className='absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4'>
										<h3 className='text-white font-medium text-lg'>{media.description}</h3>
										<span className='text-white/80 text-sm'>{getTimeAgo(media.created_at)}</span>
									</div>
					...
					<div className='mb-16'>
						<div className='flex items-center justify-between mb-6'>
							...
							<Link href='/albums' className='text-primary hover:underline text-sm'>
								See all →
							</Link>
						</div>
								...
								{mockAlbums.map((album) => (
									<Link
										...
										<div className='p-4'>
											...
											<p className='text-sm text-muted-foreground mt-1'>{formatDate(album.created_at)}</p>
										</div>
									</Link>
								))}
							</div>
						...
						<div className='lg:col-span-2'>
							<div className='bg-card rounded-xl shadow-md overflow-hidden border border-border/50'>
								<div className='flex items-center justify-between p-4 border-b border-border/50'>
									...
									<Link href='/family/invite' className='text-sm text-primary hover:underline'>
										+ Invite
									</Link>
								</div>
								...
								<div className='divide-y divide-border/50'>
									{familyMembers.map((member, index) => (
										<div key={index} className='flex items-center p-4 hover:bg-muted/50 transition-colors duration-200'>
							...
							<div className='p-4 space-y-4'>
								<div className='flex items-start gap-3'>
									...
									<div>
										...
										<p className='text-sm text-muted-foreground'>Sara added 5 new photos to "Birthday Party"</p>
								...
								<div className='flex items-start gap-3'>
									...
									<div>
										...
										<p className='text-sm text-muted-foreground'>You created "Summer Vacation 2024"</p>
								...
								<div className='flex items-start gap-3'>
									...
									<div>
										...
										<p className='text-sm text-muted-foreground'>Michael accepted your invitation</p>
						...
						<div className='bg-gradient-to-r from-primary/90 to-primary/70 rounded-xl shadow-lg overflow-hidden backdrop-blur-lg border border-primary/10 animate-in fade-in-0 duration-700 hover:shadow-xl transition-shadow'>
							<div className='p-6 text-primary-foreground relative overflow-hidden'>
								...
								<div className='flex items-start gap-4'>
									...
									<div className='flex-grow'>
										<h3 className='text-xl font-bold mb-2'>Premium Family Account</h3>
										<p className='opacity-90'>
											Your memories are safely stored with unlimited storage. Enjoy all premium features!
										</p>
								...
								<div className='flex items-start gap-4'>
									...
									<div className='flex-grow'>
										...
										<p className='text-muted-foreground'>
											Get unlimited storage, advanced organization features, and more!
										</p>
